...
# SCPN MIF Liner Core — tier-G2 device model tests
...
from physics_fixtures import ANCHOR_LINER_LENGTH_M, ANCHOR_LOW_YIELD_RADIUS_M
...
from scpn_mif_liner_core.geometry.model import BODY_NAMES
...
REFERENCE_CAD_MODEL_SHA256 = (
    "224f513f954668c72a5e81dabf9a1099064d548e951962c637ef949e60bbd1ee"
)
...
def test_every_body_agrees_with_its_tier_g1_twin() -> None:
    """The two tiers describe one device, not two similar ones."""
    for body in model().bodies:
...
def test_an_inadmissible_segment_count_is_refused_by_the_builder() -> None:
    """The tier-G1 reference is built first and applies its contract."""
    with pytest.raises(DeviceGeometryError, match="segments"):
        build_device_cad(reference_configuration(), reference_geometry(), 12)
...
def test_the_anchor_bodies_carry_the_printed_liner_dimensions() -> None:
    ...
    assert math.isclose(liner["bounding_box_max_m"][0], 0.203, rel_tol=1.0e-12)
    assert math.isclose(
        liner["bounding_box_max_m"][2] - liner["bounding_box_min_m"][2],
        ANCHOR_LINER_LENGTH_M,
        rel_tol=1.0e-12,
    )
    plasma = bodies["target_plasma"]
    assert math.isclose(
        plasma["bounding_box_max_m"][0], ANCHOR_LOW_YIELD_RADIUS_M, rel_tol=1.0e-12
    )
