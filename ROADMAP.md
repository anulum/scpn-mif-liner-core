...
- **Device configuration model** (landed 2026-08-31) — validated
  material-liner and magnetised-target objects for
  `mechanical_or_liquid_liner_mif` with hard liner-class and
  premagnetisation invariants, the specific-kinetic-energy relation, a
  slow-liner velocity-regime advisory (LINUS-class studies), canonical
  digests, and the SPO registry data pin; `computational_prototype`
  (ADR 0002, `VALIDATION.md#device-configuration-model`).
  Synchronisation contracts and repetition facets remain future work
  under the same capability.

## Planned (no implementation exists; ordering is not a commitment)
1. **Diagnostic and clock semantics** — declared synchronisation, liner,
   compression, burn, and thermal-cycle channels with dual clock
...
2. **Safety-envelope declaration** — machine-readable operational envelope
   (drive, liner, thermal bounds) consumed by the CONTROL adapter
   contract.
...
4. **Solver seam consumption** — versioned consumption of exact
   `SCPN-FUSION-CORE` seams for slow-compression and interface surfaces,
...
MagLIF pulsed-power liners, plasma-jet liners, target-plasma physics, the
pulsed FRC merge-compression workflow, magnetic-confinement devices,
