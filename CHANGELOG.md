...
- Diagnostic and clock semantics model
  (`src/scpn_mif_liner_core/observability.py`), the second implemented
...
- Device configuration model (`src/scpn_mif_liner_core/`), the first implemented
...
- Machine-readable domain manifest `reactor-domain.json` binding the project
  ...
  (configuration `mechanical_or_liquid_liner_mif`).
