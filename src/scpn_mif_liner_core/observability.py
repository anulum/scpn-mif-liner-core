...
# SCPN MIF Liner Core — diagnostic and clock semantics
...
identities for the mechanical and liquid-liner configuration this
...
from scpn_mif_liner_core.configuration import ConsistencyFinding
from scpn_mif_liner_core.errors import DiagnosticPlanError
...
ASYMMETRY_BAND_HZ: Final = (1.0e03, 1.0e07)
ARRIVAL_TIMING_CEILING_S: Final = 1.0e-06
        ...
        for configuration in self.configurations:
            if configuration not in OWNED_CONFIGURATIONS:
                raise DiagnosticPlanError(
                    ...
                    f"{configuration!r} is not owned by SCPN-MIF-LINER-CORE"
                )
...
APPLICABLE_CANDIDATES: Final = (
    CandidateProfile(
        ...
        configurations=("mechanical_or_liquid_liner_mif",),
    ...
    def __post_init__(self) -> None:
        ...
        if self.candidate_id not in _CANDIDATE_INDEX:
            raise DiagnosticPlanError(
                ...
                "applicable to SCPN-MIF-LINER-CORE"
            )
        if not self.reason:
...
class DiagnosticPlan:
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            Findings from documented liner-MIF diagnostic practice; empty
