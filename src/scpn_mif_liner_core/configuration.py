...
# SCPN MIF Liner Core — device configuration container
...
class DeviceConfiguration:
    """Validated material-liner-MIF device configuration.
    ...
    liner
        Validated material liner.
    target
        Validated magnetised target.
    registry
    ...
    def __post_init__(self) -> None:
        ...
        if self.identifier not in OWNED_CONFIGURATIONS:
            raise DeviceConfigurationError(
                ...
                f"SCPN-MIF-LINER-CORE; owned: {OWNED_CONFIGURATIONS!r}"
            )
    ...
    def consistency_report(self) -> tuple[ConsistencyFinding, ...]:
        ...
        tuple of ConsistencyFinding
            ...
            the implosion velocity stays in the slow material-liner
            regime. Findings are advisory instruments, not machine
            claims.
        """
        findings: list[ConsistencyFinding] = []
        velocity = self.liner.implosion_velocity_km_s
        if velocity > SLOW_LINER_MAX_VELOCITY_KM_S:
            findings.append(
                ConsistencyFinding(
                    field="liner.implosion_velocity_km_s",
                    message=(
                        f"implosion velocity {velocity:.1f} km/s is beyond "
                        f"the slow material-liner regime bound "
                        f"{SLOW_LINER_MAX_VELOCITY_KM_S:.0f} km/s"
                    ),
                )
            )
    ...
    def to_record(self) -> dict[str, Any]:
        ...
        return {
            ...
            "liner": {
                "kind": self.liner.kind,
                "inner_radius_m": self.liner.inner_radius_m,
                "implosion_velocity_km_s": (self.liner.implosion_velocity_km_s),
            },
            "target": {
                "initial_field_t": self.target.initial_field_t,
            },
            "registry": {
...
def configuration_from_record(record: Any) -> DeviceConfiguration:
    ...
    known = {"identifier", "liner", "target", "registry"}
    ...
    return DeviceConfiguration(
        ...
        liner=MaterialLiner(
            kind=_string(liner, "kind"),
            inner_radius_m=_number(liner, "inner_radius_m"),
            implosion_velocity_km_s=_number(liner, "implosion_velocity_km_s"),
        ),
        target=MagnetisedTarget(
            initial_field_t=_number(target, "initial_field_t"),
        ),
        registry=RegistryBinding(
